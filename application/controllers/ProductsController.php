...
class ProductsController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Products']))
		{
			...
			$uploadedIcon=CUploadedFile::getInstance($model,'prodIcon');
			$iconfile = "{$Refpage}-{$uploadedIcon}"; 
			if ($uploadedIcon !== null)
			$model->prodIcon =  $iconfile ;
			$GrpModel = ProductGroup::model()->findByPk($model->grpID);
			...
			if($model->save()){
				...
				Yii::app()->db->createCommand("Update configurations set Version=Version+1")->execute();
				 $this->redirect(array('view','id'=>$model->prodID));
			}
		}
