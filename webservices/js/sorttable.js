...
sorttable = {
	...
	makeSortable: function (table) {
		...
		if (table.alreadysorted === true) return;
		...
		if (table.getElementsByTagName('thead').length == 0) {
			...
			the.appendChild(firstrow);
			table.insertBefore(the, table.firstChild);
			if (stIsIE && table.rows.length > 1) { //ie8 bug that show extra bottom row... lets just reappend first lchild row
				tmprow = table.rows[1];
				table.tBodies[0].insertBefore(tmprow, tmprow);
			}
		}
		...
		table.alreadysorted = true;
	},

	sortColumn: function (headerCell) {
		if (sorttable.onSort) { sorttable.onSort(headerCell.sorttable_columnindex); }
		...
		if (headerCell.className.search(/\bsorttable_sorted\b/) != -1) {
			...
			sortrevind.innerHTML = stIsIE ? '&nbsp<span style="font: 80% webdings">5</span>' : '&nbsp;&#x25B4;';
		...
		if (headerCell.className.search(/\bsorttable_sorted_reverse\b/) != -1) {
			...
			sortfwdind.innerHTML = stIsIE ? '&nbsp<span style="font: 80% webdings">6</span>' : '&nbsp;&#x25BE;';
			headerCell.appendChild(sortfwdind);
			return;
		}

		// remove sorttable_sorted classes
		theadrow = headerCell.parentNode;
		...
		rows = headerCell.sorttable_tbody.rows;
